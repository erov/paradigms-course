...
> Реализация: [GenericTabulator.java](/hw05-JS-FunctionalExpression/functionalExpression.js)
...
4. _Сложный вариант_. Требуется дополнительно написать функцию `parse`, осуществляющую разбор выражений, записанных в [https://ru.wikipedia.org/wiki/Обратная_польская_запись](обратной польской записи). Например, результатом
```
parse("x x 2 - * x * 1 +")(5)
```
